...
def noisy_y_grover(nqubits, operate_times, delta):
    state = QuantumState(nqubits)
    state.set_zero_state()
    ...
    # グローバーのアルゴリズムの実行
    Hadamard = operation.make_y_noisy_Hadamard(nqubits, delta)
    U_w = operation.make_U_w(nqubits)
    U_s = operation.make_U_s(nqubits)
...
def noisy_z_grover(nqubits, operate_times, delta):
    state = QuantumState(nqubits)
    state.set_zero_state()
    ...
    # グローバーのアルゴリズムの実行
    Hadamard = operation.make_z_noisy_Hadamard(nqubits, delta)
    U_w = operation.make_U_w(nqubits)
    U_s = operation.make_U_s(nqubits)
